...
# ——— Low‐level notification thresholds & state ———
LOW_THRESHOLD  = 30.0   # notify when level ≤ this
HIGH_THRESHOLD = 50.0   # reset notification once level ≥ this
_low_notified  = False  # internal flag
...
def set_food_level():
    ...
    try:
        ...
        # Reset the low‐level alert once we refill above HIGH_THRESHOLD
        if level >= HIGH_THRESHOLD and _low_notified:
            _low_notified = False
            print("[INFO] Level above high threshold; will alert again on low")

        # Send a one-time alert when we first drop to or below LOW_THRESHOLD
